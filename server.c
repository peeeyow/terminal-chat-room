...
int main(int argc, char *argv[]){
    ...
    int server_fd, client_fd, test_fd, port, error_checking, i, j, size, client_count = 0, fd_no_max, serving, opt = 1;//server_fd = server itself, client
    struct sockaddr_in server_addr, client_addr;//struct for the server address info, and the clients address info
    message_t parse;
    pid_t stall;
    ...
    server_fd = socket(AF_INET, SOCK_STREAM, 0);// creation of server's file descriptor
    if(server_fd < 0){
    ...
    for(;;){// start
        ...
        for(serving = 0; serving <= fd_no_max; serving++){
            if(FD_ISSET(serving, &temp)){ // searches for fd that are members of the fd_set
                if(serving == server_fd){// accepts new client until 4
                    ...
                    if(client_count < 4){
                        ...
                        if(client_fd < 0){
                            ...
                            error_checking = send(client_fd, "SERVER :Please Enter your Username:", sizeof("SERVER: Please Enter your Username:"), 0);
                            if(error_checking == -1){
                                perror("Error send()");
                                exit(1);
                            }
                                ...
                                if(j == 0){
                                    ...
                                    for(i = 0; i < 4; i++){
                                        if(client_list[i].socket_name == 0){
                                            ...
                                            client_count++;//increments the count because a client has been added
                                            FD_SET(client_fd, &core);// adds the new client to the fd_set
                                            if(client_fd > fd_no_max){// renews the highest fd number
                                                fd_no_max = client_fd;
                                            }
                                            break;
                                        }
                                    ...
                                    error_checking = send(client_fd, "Duplicate Username :(.\nEnter Again.", sizeof("Duplicate Username :(.\nEnter Again."), 0);
                                    if(error_checking == -1){
                                        perror("Error send()");
                                        exit(1);
                                    }
                    ...
                    if((error_checking = recv(serving, &size, sizeof(size), 0)) <= 0){
                        if(error_checking == 0){// DISCONNECTED CLIENTS
                            for(i = 0; i < 4; i++){// search for the name of the disconnected client
                        ...
                        for(i = 0; i < client_count; i++){// search for the name of the disconnected client
                            if(client_list[i].socket_name == serving){
                        ...
                        if((error_checking = recv(serving, buff, size, 0)) <= 0){
                            ...
                            if(strcmp(parse.command, "/pm") == 0){//pm
                                for(i = 0, j = 0; i < client_count; i++){
                                    if((strcmp(parse.name, client_list[i].user_name) == 0) && (client_list[i].socket_name != serving)){
                                        j = 1;
                                        error_checking = send(client_list[i].socket_name, name, sizeof(size), 0);
                                        if(error_checking == -1){
                                            perror("Error errno");
                                            exit(1);
                                        }
                                        size = strlen(parse.message) + 1;
                                        error_checking = send(client_list[i].socket_name, &size, sizeof(size), 0);
                                        if(error_checking == -1){
                                            perror("Error errno");
                                            exit(1);
                                        }
                                        error_checking = send(client_list[i].socket_name, parse.message, size, 0);
                                        if(error_checking == -1){
                                            perror("Error errno");
                                            exit(1);
                                        }
                                ...
                                if(stall == 0){
                                    ...
                                    for(i = 0; i < 4; i++){
                                        if(FD_ISSET(client_list[i].socket_name, &core)){
                                            if ((client_list[i].socket_name != server_fd) && (client_list[i].socket_name != serving)) { //must not send to self and server;
                                                if(send(client_list[i].socket_name, name, sizeof(name), 0) == -1){
                                                    perror("Error send()");
                                                }
                                                size = strlen(parse.message) + 1;
                                                if(send(client_list[i].socket_name, &size, sizeof(size), 0) == -1){
                                                    perror("Error send()");
                                                }
                                                if(send(client_list[i].socket_name, parse.message, size, 0) == -1){
                                                    perror("Error send()");
                                                }
                                            }
                                        }
                                ...
                                if(name_checking(parse.name) == 0){
                                    ...
                                    if(j == 0){
                                        ...
                                        if(send(serving, "SERVER: Username change was succesful.", size, 0) == -1){
                                            perror("Error send()");
                                        }
                                    }
                                    else if(j == 1){
                                        ...
                                        size = sizeof("SERVER: Username change was NOT succesful.") + 1;
                                        if(send(serving, &size, sizeof(size), 0) == -1){
                                            perror("Error send()");
                                        }
                                        if(send(serving, "SERVER: Username change was NOT succesful.", size, 0) == -1){
                                            perror("Error send()");
                                        }
                                    }
                                }
