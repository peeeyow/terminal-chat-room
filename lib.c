...
message_t parser(char *string, int size){// pure parsing checking of validity must be on client
    message_t parsed;
    ...
    if(strcmp(parsed.command, "/pm") == 0){
        ...
        parsed.message = (char *)malloc(size*sizeof(char));//buff and buff2 are now useless can be freed
        checker = strchr(string, '"');
        if(checker != NULL){
            strcpy(buff, checker);
        }
    ...
    while(string[i] != '\0'){// parsing
        if(!(isalnum(string[i]))){
            if(!(isspace(string[i]))){
                if(string[i] != '.'){
                    if(string[i] != '!'){
                        puts("SERVER: Messages should contain the following characters only:");
